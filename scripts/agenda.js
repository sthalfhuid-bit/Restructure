...
function addAppointment(data) {
  ...
  const type = data.type || appointmentTypeFromCategory(category).eventType;
  ...
  const appointment = {
    ...
    time: data.time || "",
    type,
    meta: data.meta || appointmentTypeFromCategory(category).label || typeLabel(type),
    category,
...
function addDayItem(data) {
  ...
  const dayItem = {
    ...
    type: data.type || "task",
    time: data.time || "",
    category: data.category || categoryFromEvent({ title: text, type: data.type || "task" }),
...
function renderWeekOverview(events) {
  ...
  if (!weekEvents.length) {
    DOM.weekInsight.textContent = "Nog geen afspraken. Voeg iets toe via Brain Dump of de + knop.";
    DOM.deadlineCount.textContent = 0;
    DOM.appointmentCount.textContent = 0;
    DOM.sportCount.textContent = 0;
    DOM.schoolCount.textContent = 0;
    DOM.weekDeadlines.innerHTML = "";
    DOM.weekAppointments.innerHTML = listMarkup(["Nog geen afspraken deze week."]);
  ...
  DOM.deadlineCount.textContent = allDeadlines.length;
  DOM.appointmentCount.textContent = allAppointments.length;
  DOM.sportCount.textContent = allSports.length;
  DOM.schoolCount.textContent = allSchool.length;
...
function renderAgenda(events) {
  ...
  DOM.calendarTitle.textContent = weekStart.toLocaleDateString("nl-NL", { month: "long", year: "numeric" });
  ...
  return `
    <article class="day-card ${isQuiet ? "rest-day" : ""} ${iso === AppState.selectedDate ? "selected" : ""}" data-date="${iso}" tabindex="0">
      <div class="day-heading">
        <span class="mobile-day-label">${date.toLocaleDateString("nl-NL", { weekday: "short" }).replace(".", "").slice(0, 1)}</span>
        <h3>${date.getDate()}</h3>
...
function renderSelectedDay(events) {
  ...
  DOM.selectedDayTitle.textContent = date.toLocaleDateString("nl-NL", { weekday: "long", day: "numeric", month: "long" });
  DOM.selectedDayCount.textContent = `${items.length} ${items.length === 1 ? "item" : "items"}`;
...
function renderPersonalAppointments(events) {
  ...
  DOM.personalAppointmentList.innerHTML = upcoming.length
    ? upcoming
        ...
        .join("")
    : `<li>Nog geen afspraken. Voeg iets toe via Brain Dump of de + knop.</li>`;
}

...
function bindBrainDumpEvents() {
  DOM.overviewGrid.addEventListener("click", (event) => {
    ...
    if (addProposal) {
      ...
      addProposal.closest(".proposal-actions").innerHTML = `<span class="quiet-note">Toegevoegd aan Agenda</span>`;
      return;
    }
    ...
    item.insertAdjacentHTML(
      "beforeend",
      `<div class="small-step"><span>Kleine eerste stap</span><p>${escapeHtml(step)}</p><button class="soft-button" type="button" data-hide-small-step>Terug naar origineel</button></div>`
    );
  });
...
function bindAgendaEvents() {
  DOM.appointmentForm.addEventListener("submit", (event) => {
    ...
    if (!text) {
      DOM.appointmentFeedback.textContent = "Typ rustig een afspraak in gewone taal.";
      return;
    }
    ...
    DOM.appointmentFeedback.textContent = `Toegevoegd: ${formatEventLine(savedAppointment)}.`;
  });
  ...
  DOM.calendarPanel.addEventListener("touchstart", (event) => {
    touchStartX = event.touches[0]?.clientX || 0;
  }, { passive: true });

  DOM.calendarPanel.addEventListener("touchend", (event) => {
  ...
  DOM.enableReminders.addEventListener("click", async () => {
    if (typeof Notification === "undefined") {
      DOM.appointmentFeedback.textContent = "Je browser ondersteunt geen meldingen, maar de herinneringen blijven zichtbaar in de agenda.";
      return;
    }
